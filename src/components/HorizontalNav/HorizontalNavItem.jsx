...
import HorizontalNavItemLabel from './HorizontalNavItemLabel';
import classNames from 'classnames';
import uniqueId from 'lodash.uniqueid';

export class HorizontalNavItem extends React.PureComponent {
  constructor(props) {
    super(props);
    this.handleLabelClick = this.handleLabelClick.bind(this);
    this.id = this.props.id || uniqueId('HorizontalNavItem_');
  ...
  /**
   * Called when HorizontalNavItemLabel is clicked. Since the "label" could be
  ...
  /**
   * Note: This event handler will only get called when the HorizontalNavItemLabel
  ...
  subnavItems() {
    if (this.props.url) {
      // Since the HorizontalNavItemLabel will just toggle the subnav, we
  ...
  render() {
    const classes = classNames('ds-c-Horizontal-nav__item', this.props.className);

    return (
      <li className={classes}>
        <HorizontalNavItemLabel
...
HorizontalNavItem.defaultProps = {
  ...
  // HorizontalNavItemLabel. We define them here so they show in the docs.
...
HorizontalNavItem.propTypes = {
  /**
   * @hide-prop This gets passed through from the parent HorizontalNav to a nested HorizontalNav
   */
   ...
   * `id`, `url`.
   *
   * This takes precedence over the `HorizontalNav` `onLinkClick` prop
   */
  ...
  id: PropTypes.string,
  /**
   * An array of nested `HorizontalNavItem` data objects to be rendered in a
   * sub-navigation list.
   */
